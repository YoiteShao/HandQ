...
# ── Coding-mode hint ────────────────────────────────────────────────────────
# Appended to effective_goal by CodingContextProvider when the planner
# declares "coding" in step.tools_required.  Do NOT duplicate guidance that
# already lives in tool usage_guides (edit/read/write/shell):
#   • exact-match / 3-5-line context for edit         → already in edit usage_guide
#   • read-before-write & batch reads                 → already in read usage_guide
#   • prefer dedicated tools over shell               → already in shell usage_guide
#   • dangerous-command refusal (rm -rf /, mkfs, …)   → already enforced by ShellTool
# This block covers ONLY behavioral / semantic rules tools cannot teach.
